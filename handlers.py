...
async def start(update, context):
    ...
    await context.bot.send_message(
        ...
        text=f"Connected {user_info}.{chat_info}"
    )


async def forward_to_chat(update, context):
    """{ 
        'message_id': 5, 
        'date': 1605106546, 
...
async def forward_to_user(update, context):
    """{
...
def setup_dispatcher(app):
    ...
    app.add_handler(MessageHandler(filters.Chat(TELEGRAM_SUPPORT_CHAT_ID) & filters.REPLY, forward_to_user))
